...
def update_scrims_data(worksheet, series_list, debug_logs, progress_bar):
    ...
    for i, s_summary in enumerate(series_list):
        ...
        # 3. Пытаемся извлечь ID игры (g_id) из s_data
        g_id = None
        series_state_data = s_data.get("seriesState") # Проверяем новый путь первым
        ...
        # Если g_id НЕ найден, пропускаем
        if not g_id:
             debug_logs.append(f"Warn: No game ID (g_id) found in s_data for series {s_id} (identified as ours by s_data). Skipping.")
        ...
        if not is_our_scrim_gdata:
            ...
            continue
        # stats["gms_found_in_gdata"] не нужен, т.к. gms_count ниже отразит успешную обработку
        ...
        for team_state in game_teams_data:
            ...
            # --- Блок if is_our_team_in_game (с корректными отступами) ---
